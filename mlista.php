...
if(empty($_SESSION['mid'])){
 Print '<script>alert("ACCESS DENIED. PLEASE LOGIN");</script>';
  Print '<script>window.location.assign("index.html");</script>';
    ...
    $result=mysqli_query($dbc,"SELECT * FROM `form` where mid_fk = '$_SESSION[mid]' and sid_fk is not null and tid_fk is null order by ddate desc ") or die("There are no records to display ... \n" . mysql_error()); 

 if(isset($_POST['sort'])){
  if($_POST['sort'] == 'title'){
    $result=mysqli_query($dbc,"SELECT * FROM `form` where mid_fk = '$_SESSION[mid]' and sid_fk is not null and tid_fk is null order by dname") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }
  if($_POST['sort'] == 'status'){

    $result=mysqli_query($dbc,"SELECT * FROM `form` where mid_fk = '$_SESSION[mid]' and sid_fk is not null and tid_fk is null order by dstats") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }
  if($_POST['sort']=='date'){
  ...
  if($_POST['sort']=='id'){
    ...
    $result=mysqli_query($dbc,"SELECT * FROM `form` where mid_fk = '$_SESSION[mid]' and sid_fk is not null and tid_fk is null order by did") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }
}
...
<tr>
    ...
    if($row['dstats'] == "APPROVED" || $row['dstats'] == "REJECTED" || $row['dstats'] == "CANCELLED")
      { ?>
    <td colspan="2"><center><a href="mlistadet.php?id=<?php echo $row['did'] ?>" class="btn btn-default" role="button">Details</a></center></td>
    <?php } 

    else {?>
       <td><a href="mlistadet.php?id=<?php echo $row['did'] ?>" class="btn btn-default" role="button">Details</a></td>
    <td><a href="mlistaupd.php?id=<?php echo $row['did'] ?>" class="btn btn-default" role="button">Update</a></td>
  <?php }?>
 </tr>   
<?php $i++; }
?>
