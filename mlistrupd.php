...
$id = $_GET['id'];
     $sql = "select * from form where did='$id'";
...
$status1 = addslashes( $_POST['mstats'] );
...
$sql = "UPDATE form SET dstats='$status', m='$m' , dcomm='$comment' WHERE did = '$id'";
        $result=mysqli_query($dbc,$sql);
...
if($result) //success  
     {
      ...
      Print '<script>window.location.assign("mlistr.php?id='.$id.'");</script>'; 
    }
    else //unsuccess  
    {
      mysqli_rollback($dbc);
...
<form action="mlistrupd.php?id=<?php echo $id; ?>" method="post" name="form1"> 
<table align="center">
<div class="box" >
...
<tr>
  ...
  document.getElementById('mstats').value = "<?php echo $status;?>";
  </script>
    </td>
</tr>
<tr>
