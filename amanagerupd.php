...
$query1 = "SELECT * FROM `manager` where mid='$id'";
...
$did= addslashes( $_POST['fid'] );
$posit= addslashes( $_POST['fposit'] );

$sql = "UPDATE manager SET mpass='$pass', mid='$did', mname='$name', memail='$email', mposit='$posit' WHERE mid = '$id'";
...
if($result) //success  
     {
      ...
      Print '<script>window.location.assign("alistmanager.php?id='.$id.'");</script>'; 
    }
    else //unsuccess  
    {
      mysqli_rollback($dbc);
...
<form action="amanagerupd.php?id=<?php echo $id; ?>" method="post" name="form1" onclick="return validateFOrm()"> 
<table align="center">
<div class="box" >
...
<tr>
  <td> ID:</td>
  <td>
    <input  class="form-control" type="text" style="background-color:#99s9999; font-weight:bold;" name="fid" maxlength="10" value='<?=$row['mid'];?>' required>
  </td>
</tr>
<tr>
  <td>Name:</td>
