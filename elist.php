...
if(empty($_SESSION['eid'])){
 Print '<script>alert("ACCESS DENIED. PLEASE LOGIN");</script>';
  ...
  $result=mysqli_query($dbc,"SELECT * FROM `form` where exec is not null order by ddate desc ") or die("There are no records to display ... \n" . mysql_error()); 

 if(isset($_POST['sort'])){
  if($_POST['sort'] == 'title'){
    $result=mysqli_query($dbc,"SELECT * FROM `form` where exec is not null order by dname") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }
  if($_POST['sort'] == 'status'){

    $result=mysqli_query($dbc,"SELECT * FROM `form` where exec is not null order by dstats") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }
  if($_POST['sort']=='date'){
  ...
  if($_POST['sort']=='id'){
    ...
    $result=mysqli_query($dbc,"SELECT * FROM `form` where exec is not null order by did") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }
...
<?php
  ...
  {
    if( ($row['sid_fk'] != null && $row['t'] == "APPROVED" )|| ($row['mid_fk'] != null && $row['t']  == "APPROVED") || ($row['tid_fk'] != null && $row['exec'] != null && $row['sid_fk'] == null && $row['mid_fk'] == null)){
  ?>    
  <tr>
      <td><?php echo $i ?></td>
    ...
    if($row['dstats'] == "APPROVED" || $row['dstats'] == "REJECTED" || $row['dstats'] == "CANCELLED")
      { ?>
    <td colspan="2"><center><a href="elistdet.php?id=<?php echo $row['did'] ?>" class="btn btn-default" role="button">Details</a></center></td>
    <?php } 

    else {?>
       <td><a href="elistdet.php?id=<?php echo $row['did'] ?>" class="btn btn-default" role="button">Details</a></td>
    <td><a href="elistupd.php?id=<?php echo $row['did'] ?>" class="btn btn-default" role="button">Update</a></td>
  <?php }?>
</tr>
<?php $i++;}} 
