...
$result=mysqli_query($dbc,"SELECT * FROM `exec`")
or die("There are no records to display ... \n" . mysql_error()); 


 if(isset($_POST['sort'])){
  if($_POST['sort'] == 'name'){
    $result=mysqli_query($dbc,"SELECT * FROM `exec` order by ename") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }

  if($_POST['sort']=='id'){

    $result=mysqli_query($dbc,"SELECT * FROM `exec` order by eid") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }
}
...
<div class="w3-container w3-dark-gray" width="100">
  <center><h1>EXECUTIVE LIST</h1></center>
</div>
...
<tr>
    ...
    <td><a href="aexecdet.php?id=<?php echo $row['eid'] ?>" class="btn btn-default" role="button">Details</a></td>
    <td><a href="aexecupd.php?id=<?php echo $row['eid'] ?>" class="btn btn-default" role="button">Update</a></td>
    <td><a href="aexecdel.php?id=<?php echo $row['eid'] ?>" class="btn btn-default" role="button">Delete</a></td>
</tr>
