...
$check1= "SELECT * from form f , staff s , manager m, topm t where f.did='$id' and s.sid = f.sid_fk and m.mid = f.mid_fk and t.tid =f.tid_fk";
$check = mysqli_query($dbc, $check1);
$c = mysqli_fetch_assoc($check);

if ( $c['sid_fk'] != null)
{
     $sql = "SELECT * from form f , staff s , manager m, topm t where f.did='$id' and s.sid = f.sid_fk and m.mid = f.mid_fk and t.tid =f.tid_fk and exec is not null";
     $result = mysqli_query($dbc, $sql);
     $row = mysqli_fetch_assoc($result);
}
else
{
  $sql = "SELECT * from form f , manager m, topm t where f.did='$id' and m.mid = f.mid_fk and t.tid =f.tid_fk and exec is not null";
     $result = mysqli_query($dbc, $sql);
     $row = mysqli_fetch_assoc($result);
...
<tr>
    <td>Comment:</td>
    <td><div class="box" ><?php echo $row['dcomm']; ?></div></td>
</tr>
<tr>
    <td>Date Created:</td>
