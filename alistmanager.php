...
$result=mysqli_query($dbc,"SELECT * FROM `manager`")
or die("There are no records to display ... \n" . mysql_error()); 

 if(isset($_POST['sort'])){
  if($_POST['sort'] == 'name'){
    $result=mysqli_query($dbc,"SELECT * FROM `manager` order by mname") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }

  if($_POST['sort']=='id'){

    $result=mysqli_query($dbc,"SELECT * FROM `manager` order by mid") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }
}
...
<div class="w3-container w3-dark-gray" width="100">
  <center><h1>MANAGER LIST</h1></center>
</div>
...
<tr>
    ...
    <td><a href="amanagerdet.php?id=<?php echo $row['mid'] ?>" class="btn btn-default" role="button">Details</a></td>
    <td><a href="amanagerupd.php?id=<?php echo $row['mid'] ?>" class="btn btn-default" role="button">Update</a></td>
    <td><a href="amanagerdel.php?id=<?php echo $row['mid'] ?>" class="btn btn-default" role="button">Delete</a></td>
</tr>
