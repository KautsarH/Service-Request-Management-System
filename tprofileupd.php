...
$sql = "UPDATE topm SET tname='$name', tpass='$pass', temail='$email' WHERE tid = '$id'";
...
if($result) //success  
     {
      ...
      Print '<script>window.location.assign("tprofile.php?id='.$id.'");</script>'; 
    }
    else //unsuccess  
    {
      mysqli_rollback($dbc);
...
<form action="tprofileupd.php?id=<?php echo $id; ?>" method="post"  name="form1" onsubmit="return validateForm()"> 
<table align="center">
<div class="box" >
...
<tr>
  ...
  <td><fieldset disabled>
    <input  class="form-control" type="text" style="background-color:#99s9999; font-weight:bold;" name="fid" maxlength="10" value='<?=$row['tid'];?>' required></fieldset>
  </td>
</tr>
<tr>
  <td>Name:</td>
  <td>
    <input  class="form-control" type="text" style="background-color:#99s9999; font-weight:bold;" name="fname" maxlength="20" value='<?=$row['tname'];?>' required>
  </td>
</tr>
<tr>
  <td>Password:</td>
  <td>
    <input  class="form-control" type="password" style="background-color:#99s9999; font-weight:bold;" name="fpass" maxlength="20" value='<?=$row['tpass'];?>' required>
  </td>
</tr>
<tr>
  <td>Email:</td>
  <td>
    <input  class="form-control" type="text" style="background-color:#99s9999; font-weight:bold;" name="femail" maxlength="30" value='<?=$row['temail'];?>' required>
  </td>
</tr>
