...
if(empty($_SESSION['tid'])){
 Print '<script>alert("ACCESS DENIED. PLEASE LOGIN");</script>';
  ...
  $result=mysqli_query($dbc,"SELECT * FROM `form` where tid_fk = '$_SESSION[tid]' and t is not null and exec is not null order by ddate desc ") or die("There are no records to display ... \n" . mysql_error()); 

 if(isset($_POST['sort'])){
  if($_POST['sort'] == 'title'){
    $result=mysqli_query($dbc,"SELECT * FROM `form` where tid_fk = '$_SESSION[tid]' and t is not null and exec is not null order by dname") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }
  if($_POST['sort'] == 'status'){

    $result=mysqli_query($dbc,"SELECT * FROM `form` where tid_fk = '$_SESSION[tid]' and t is not null and exec is not null order by dstats") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }
  if($_POST['sort']=='date'){
  ...
  if($_POST['sort']=='id'){
    ...
    $result=mysqli_query($dbc,"SELECT * FROM `form` where tid_fk = '$_SESSION[tid]' and t is not null and exec is not null order by did") or die("There are no records to display ... \n" . mysql_error()); 
    mysqli_commit($dbc);
  }
}
...
<tr>
    ...
    if($row['dstats'] == "APPROVED" || $row['dstats'] == "REJECTED" || $row['dstats'] == "CANCELLED")
      { ?>
    <td colspan="2"><center><a href="tlistrdet.php?id=<?php echo $row['did'] ?>" class="btn btn-default" role="button">Details</a></center></td>
    <?php } 

    else {?>
       <td><a href="tlistrdet.php?id=<?php echo $row['did'] ?>" class="btn btn-default" role="button">Details</a></td>
    <td><a href="tlistrupd.php?id=<?php echo $row['did'] ?>" class="btn btn-default" role="button">Update</a></td>
  <?php }?>
</tr>
